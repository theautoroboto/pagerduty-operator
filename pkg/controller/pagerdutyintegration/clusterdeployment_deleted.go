...
func (r *ReconcilePagerDutyIntegration) handleDelete(pdclient pd.Client, pdi *pagerdutyv1alpha1.PagerDutyIntegration, cd *hivev1.ClusterDeployment) error {
	...
	if deletePDService {
		...
		if err != nil {
			...
			// delete it
			r.reqLogger.Info("Deleting PD ConfigMap", "Cluster deployment Namespace", cd.Namespace, "Name", configMapName)
			err = utils.DeleteConfigMap(configMapName, cd.Namespace, r.client, r.reqLogger)

			if err != nil {
				r.reqLogger.Error(err, "Error deleting ConfigMap", "Cluster deployment Namespace", cd.Namespace, "Name", configMapName)
			}
		}
	}
	// find the pd secret and delete id
	r.reqLogger.Info("Deleting PD secret", "Cluster deployment Namespace", cd.Namespace, "Name", secretName)
	err = utils.DeleteSecret(secretName, cd.Namespace, r.client, r.reqLogger)
	if err != nil {
		r.reqLogger.Error(err, "Error deleting Secret", "Cluster deployment Namespace", cd.Namespace, "Name", secretName)
	}

	// find the PD syncset and delete it
	r.reqLogger.Info("Deleting PD SyncSet", "Cluster deployment Namespace", cd.Namespace, "Name", secretName)
	err = utils.DeleteSyncSet(secretName, cd.Namespace, r.client, r.reqLogger)

	if err != nil {
		r.reqLogger.Error(err, "Error deleting SyncSet", "Cluster deployment Namespace", cd.Namespace, "Name", secretName)
	}

	if utils.HasFinalizer(cd, finalizer) {
		r.reqLogger.Info("Deleting PD finalizer from ClusterDeployment", "Cluster deployment Namespace", cd.Namespace, "Name", cd.Name)
		...
		if err := r.client.Patch(context.TODO(), cd, baseToPatch); err != nil {
			r.reqLogger.Error(err, "Error deleting Finalizer from cluster deployment", "Cluster deployment Namespace", cd.Namespace, "Name", cd.Name)
	...
	if utils.HasFinalizer(cd, config.LegacyPagerDutyFinalizer) {
		r.reqLogger.Info("Deleting old PD finalizer from ClusterDeployment", "Cluster deployment Namespace", cd.Namespace, "Name", cd.Name)
